...
class HabitEntriesService:
    ...
    async def get_by_habit(self, user_id, habit_id) -> list[HabitEntryResponse]:
        ...
        return [HabitEntryResponse(user_id=entry.user_id, habit_id=entry.habit_id, created_on_utc=entry.created_on_utc, modified_on_utc=entry.modified_on_utc) for entry in habit_entries]
    ...
    async def add(self, user_id, request:HabitEntryRequest):
        ...
        self.session.commit()
        return HabitEntryResponse(user_id=habit_entry.user_id, 
    ...
    async def delete(self, user_id, habit_id):
        habit_entry = self.session.query(HabitEntry).filter(and_(HabitEntry.user_id == user_id, HabitEntry.habit_id == habit_id)).first()
        if not habit_entry:
            return False
